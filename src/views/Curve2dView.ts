...
export class Curve2dView implements IObserver<IBSplineR1toR2> {
    ...
    constructor(private spline: IBSplineR1toR2, private curveShaders: Curve2dShaders, private red: number, private green: number, private blue: number, private alpha: number ) {
    ...
    updateVertices() {
        ...
        let tangent = ((this.pointSequenceOnSpline[1]).subtract(this.pointSequenceOnSpline[0])).normalize(),
    ...
    renderFrame() {
        ...
        const gl = this.curveShaders.gl
        const a_Position = gl.getAttribLocation(<Curve2dShaders>this.curveShaders.program, 'a_Position')
        const fColorLocation = gl.getUniformLocation(<Curve2dShaders>this.curveShaders.program, "fColor")
