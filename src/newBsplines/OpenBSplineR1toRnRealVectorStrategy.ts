...
export class OpenBSplineR1toRnRealVectorStrategy extends AbstractOPenBSplineR1toRnStrategy implements OpenBSplineR1toRnStrategy {
    ...
    euclideanDistances(): number[] {
        ...
        for (let i = 0; i < this.openBSplineR1toRn.controlPolygon.length - 1; i += 1) {
            distances.push(this.vectorSpace.normRaw(this.vectorSpace.subtractRaw(this.openBSplineR1toRn.controlPolygon.getVector(i + 1) as RealVector, this.openBSplineR1toRn.controlPolygon.getVector(i) as RealVector)));
        }
        return distances;
    }
