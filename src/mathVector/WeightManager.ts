import { EM_NULL_WEIGHT_SUBTRACT_STRICTLY_POSITIVE_WEIGHTS, EM_WEIGHT_MANAGER_WEIGHT_TYPE_ERROR, EM_WEIGHT_SUBTRACTION_ERROR } from "../ErrorMessages/WeightManager";
...
export class WeightManager {
    ...
    setWeightStatus(weight: Weight): Weight {
        ...
        if (this.weightManagement === WeightManagement.AllPositiveWeights) {
            newWeight = new Weight(weight.weight, false);
        } else if(this.weightManagement === WeightManagement.AllStrictlyPositiveWeights) {
        ...
        } else if(this.weightManagement === WeightManagement.SomeNullWeights) {
            if(weight.weight === DEFAULT_WEIGHT_VALUE) {
    ...
    addWeights(weightV1: Weight, weightV2: Weight): Weight {
        ...
        let newWeight = new Weight(sumWeights);
        if (this.weightManagement === WeightManagement.AllStrictlyPositiveWeights) {
        ...
        } else if (this.weightManagement === WeightManagement.AllPositiveWeights) {
    ...
    isSameWeightManagement(weightV1: Weight, weightV2: Weight): boolean {
        if(this.weightManagement !== WeightManagement.SomeNullWeights) {
