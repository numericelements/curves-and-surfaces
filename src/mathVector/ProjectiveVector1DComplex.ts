...
import type { ComplexDesc, ProjectiveComplexVector1D } from "./VectorDescriptorConstructorInterface";
import { sendRangeErrorMessage } from "./VectorSpaceUtilities";
import { Weight } from "./Weight";
import type { ProjectiveComplexVectorDesc } from "./utilityTypes/VectorDescriptorTypes";


export class ProjectiveVector1DComplex extends AbstractProjectiveComplexVector<2, ProjectiveComplexVector1D> 
    implements Vector<2, ProjectiveComplexVector1D>
{

    private static readonly DIMENSION = 2 as const;
    private static readonly _vectorType = PROJECTIVECOMPLEXVECTOR1D;
    private readonly  _descriptor: ProjectiveComplexVectorDesc;
    protected  readonly _vectorSpace: ProjectiveComplexVectorSpace<2, ProjectiveComplexVector1D>;
    ...
    constructor(realOrComplexOrVectorSpace?: number | Complex | ProjectiveComplexVectorSpace<2>, imaginaryOrComplexWeightOrVectorSpace?: number | ComplexWeight | ProjectiveComplexVectorSpace<2>, realWeightOrVectorSpace?: Weight | ProjectiveComplexVectorSpace<2>, imaginaryWeight?: Weight, vectorSpace?: ProjectiveComplexVectorSpace<2>) {
        ...
        if (realOrComplexOrVectorSpace instanceof ProjectiveComplexVectorSpace && imaginaryOrComplexWeightOrVectorSpace === undefined) {
            super.checkVectorSpaceDimensionConsistency(ProjectiveVector1DComplex.DIMENSION, realOrComplexOrVectorSpace);
        ...
        if (realOrComplexOrVectorSpace instanceof Complex && imaginaryOrComplexWeightOrVectorSpace instanceof ComplexWeight) {
            ...
            super.checkVectorSpaceConsistency(ProjectiveVector1DComplex.DIMENSION, realWeightOrVectorSpace);
        ...
        if (realOrComplexOrVectorSpace instanceof Complex) {
            ...
            super.checkVectorSpaceConsistency(ProjectiveVector1DComplex.DIMENSION, imaginaryOrComplexWeightOrVectorSpace);
            const complex = realOrComplexOrVectorSpace.toDescriptor();
        ...
        if(typeof realOrComplexOrVectorSpace === 'number' && typeof imaginaryOrComplexWeightOrVectorSpace === 'number'
            ...
            super.checkVectorSpaceConsistency(ProjectiveVector1DComplex.DIMENSION, vectorSpace);
    ...
    private getDefaultVectorSpace(): ProjectiveComplexVectorSpace<2> {
        try{
            return getDefaultVectorSpace(this.spaceType, ProjectiveVector1DComplex.DIMENSION);
        } catch(error) {
            return new ProjectiveComplexVectorSpace(ProjectiveVector1DComplex.DIMENSION, true);
        }
    }

    get dimension(): 2 { return ProjectiveVector1DComplex.DIMENSION; } // Homogeneous coordinates
    get vectorType(): string { return ProjectiveVector1DComplex._vectorType; }
    ...
    get descriptor(): ProjectiveComplexVectorDesc { return cloneDescriptorProjectiveComplexVector1D(this._descriptor); }
    ...
    getCoordinate(index: number): Complex {
        if (index < 0 || index >= ProjectiveVector1DComplex.DIMENSION) {
    ...
    homogeneousTransform(): ProjectiveVector1DComplex {
        ...
        return new ProjectiveVector1DComplex(complexW.divide(this.coordinates[0]), normalizedWeight, this._vectorSpace);
    }
    ...
    toComplexVector(vectorSpace?: ComplexVectorSpace<1>): Vector1DComplex {
    ...
    toProjectiveRealVector(projectiveRealVectorSpace?: ProjectiveRealVectorSpace<3>): ProjectiveVector2DReal {
        if(projectiveRealVectorSpace !== undefined) {
            const projVect =  new ProjectiveVector2DReal(this._descriptor.coordinates[0].real, this._descriptor.coordinates[0].imaginary, this.weight.real, projectiveRealVectorSpace);
            const weiht = projVect.weight;
            const weightManagement = projVect.vectorSpace.weightManagement;
            return projVect;
        }
        return new ProjectiveVector2DReal(this._descriptor.coordinates[0].real, this._descriptor.coordinates[0].imaginary, this.weight.real, projectiveRealVectorSpace);
    }
    ...
    createVectorFromDescriptor(descriptor: ProjectiveComplexVectorDesc): this {
        return new ProjectiveVector1DComplex(descriptor.coordinates[0].real, descriptor.coordinates[0].imaginary, descriptor.coordinates[1].real, descriptor.coordinates[1].imaginary, this.vectorSpace) as this;
    }
}