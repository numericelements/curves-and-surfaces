...
import type { ComplexDesc, ComplexVector2D } from "./VectorDescriptorConstructorInterface";
import { sendRangeErrorMessage } from "./VectorSpaceUtilities";


export class Vector2DComplex extends AbstractComplexVector<2, ComplexVector2D> 
     implements Vector<2, ComplexVector2D>
{
    ...
    constructor(realOrComplexOrVectorSpace?: number | Complex | ComplexVectorSpace<2>, imaginaryOrComplex?: number | Complex, real2OrVectorSpace?: number | ComplexVectorSpace<2>, imaginary2?: number, vectorSpace?: ComplexVectorSpace<2>) {
        ...
        if (realOrComplexOrVectorSpace instanceof ComplexVectorSpace && imaginaryOrComplex === undefined) {
            super.checkVectorSpaceDimensionConsistency(Vector2DComplex.DIMENSION, realOrComplexOrVectorSpace);
        ...
        if (realOrComplexOrVectorSpace instanceof Complex && imaginaryOrComplex instanceof Complex) {
            if(real2OrVectorSpace instanceof ComplexVectorSpace) {
                super.checkVectorSpaceConsistency(Vector2DComplex.DIMENSION, real2OrVectorSpace);
        ...
        super.checkVectorSpaceConsistency(Vector2DComplex.DIMENSION, vectorSpace);
    ...
    private getDefaultVectorSpace(): ComplexVectorSpace<2> {
        try{
            return getDefaultVectorSpace(this.spaceType, Vector2DComplex.DIMENSION);
        } catch(error) {
            return new ComplexVectorSpace(Vector2DComplex.DIMENSION, true);
        }
    }

    get dimension(): 2 { return Vector2DComplex.DIMENSION; }
    get vectorType(): string { return Vector2DComplex._vectorType; }
    ...
    getCoordinate(index: number): Complex {
        if (index < 0 || index >= Vector2DComplex.DIMENSION) {
    ...
    toRealVector(realVectorSpace?: RealVectorSpace<4>): Vector4DReal {
        if(realVectorSpace !== undefined) return new Vector4DReal(this.getCoordinate(0).real, this.getCoordinate(0).imaginary, this.getCoordinate(1).real, this.getCoordinate(1).imaginary, realVectorSpace);
        return new Vector4DReal(this.getCoordinate(0).real, this.getCoordinate(0).imaginary, this.getCoordinate(1).real, this.getCoordinate(1).imaginary);
    }
    ...
    createVectorFromDescriptor(vectorDescriptor: ComplexVector2D): this {
        return new Vector2DComplex(vectorDescriptor.coordinates[0].real, vectorDescriptor.coordinates[0].imaginary, vectorDescriptor.coordinates[1].real, vectorDescriptor.coordinates[1].imaginary, this.vectorSpace) as this;
    }
}