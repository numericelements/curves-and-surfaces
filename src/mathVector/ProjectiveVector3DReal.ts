...
import { PROJECTIVEREALVECTOR3D } from "../namedConstants/VectorTypeTags";
...
import { cloneDescriptorProjectiveRealVector3D, createProjectiveRealVector3DDescriptor } from "./VectorDescriptorFactory";
import type { ProjectiveRealVector3D } from "./VectorDescriptorConstructorInterface";
import { sendRangeErrorMessage } from "./VectorSpaceUtilities";
import { Weight } from "./Weight";


export class ProjectiveVector3DReal extends AbstractProjectiveRealVector<4, ProjectiveRealVector3D> 
    implements Vector<4, ProjectiveRealVector3D>
{

    private static readonly DIMENSION = 4 as const;
    private static readonly _vectorType = PROJECTIVEREALVECTOR3D;
    private readonly _descriptor: ProjectiveRealVector3D;
    protected readonly _vectorSpace: ProjectiveRealVectorSpace<4, ProjectiveRealVector3D>;
    
    constructor();
    constructor(vectorSpace: ProjectiveRealVectorSpace<4>);
    constructor(x: number, y: number, z: number, weight: Weight, vectorSpace?: ProjectiveRealVectorSpace<4>);
    constructor(x: number, y: number, z: number, vectorSpace?: ProjectiveRealVectorSpace<4>);

    constructor(xOrVectorSpace?: number | ProjectiveRealVectorSpace<4>, y?: number, z?: number, weightOrVSpace?: Weight | ProjectiveRealVectorSpace<4>, vectorSpace?: ProjectiveRealVectorSpace<4>) {
        super();
        let strictlyPosWeight = true;
        // Case 1: no arguments
        if(xOrVectorSpace === undefined) {
            this._descriptor = createProjectiveRealVector3DDescriptor(0, 0, 0, new Weight(DEFAULT_WEIGHT_VALUE, strictlyPosWeight));
        ...
        if(xOrVectorSpace instanceof ProjectiveRealVectorSpace && y === undefined) {
            super.checkVectorSpaceDimensionConsistency(ProjectiveVector3DReal.DIMENSION, xOrVectorSpace);
        ...
        if (typeof xOrVectorSpace === 'number' && typeof y === 'number' && typeof z === 'number' && weightOrVSpace instanceof Weight) {
            super.checkVectorSpaceConsistency(ProjectiveVector3DReal.DIMENSION, vectorSpace);
            strictlyPosWeight = this.checkValidityWeightStatus(weightOrVSpace, vectorSpace as ProjectiveRealVectorSpace<4>);
            this._descriptor = createProjectiveRealVector3DDescriptor(xOrVectorSpace, y!, z!, weightOrVSpace);
        ...
        if (!(weightOrVSpace instanceof Weight) && vectorSpace === undefined) {
            super.checkVectorSpaceConsistency(ProjectiveVector3DReal.DIMENSION, weightOrVSpace);
            this._vectorSpace = weightOrVSpace ?? this.getDefaultVectorSpace();;
            if(this._vectorSpace.weightManagement === WeightManagement.AllPositiveWeights) strictlyPosWeight = false;
            this._descriptor = createProjectiveRealVector3DDescriptor(xOrVectorSpace, y!, z!, new Weight(DEFAULT_WEIGHT_VALUE, strictlyPosWeight));
        } else {
            const error = sendRangeErrorMessage(this.constructor.name, 'constructor', EM_VECTORSPACE_PARAMETERS_INCOMPATIBLE);
            throw new RangeError(error.generateMessageString());
        }
    ...
    private getDefaultVectorSpace(): ProjectiveRealVectorSpace<4> {
        try{
            return getDefaultVectorSpace(this.spaceType, ProjectiveVector3DReal.DIMENSION);
        } catch(error) {
            return new ProjectiveRealVectorSpace(ProjectiveVector3DReal.DIMENSION, true);
        }
    }
    
    get dimension(): 4 { return ProjectiveVector3DReal.DIMENSION; }
    get vectorType(): string { return ProjectiveVector3DReal._vectorType; }
    get vectorSpace(): ProjectiveRealVectorSpace<4, ProjectiveRealVector3D> { return this._vectorSpace; }
    get coordinates(): readonly number[] { return this.homogeneousCoordinates; }
    get descriptor(): ProjectiveRealVector3D { return cloneDescriptorProjectiveRealVector3D(this._descriptor); }
    ...
    getCoordinate(index: number): number {
        if (index === ProjectiveVector3DReal.DIMENSION - 1) {
    ...
    homogeneousTransform(tolerance?: number): this {
        ...
        const newDescriptor: ProjectiveRealVector3D = createProjectiveRealVector3DDescriptor(normalizedCoord[0], normalizedCoord[1], normalizedCoord[2],
            new Weight(DEFAULT_WEIGHT_VALUE, this.weight.strictlyPositive));
        return this.createVectorFromDescriptor(newDescriptor);
    }

    add(other: ProjectiveVector3DReal): this {
        const result = super.add(other);
        return this.createVectorFromDescriptor(result.descriptor);
    }

    subtract(other: ProjectiveVector3DReal): this {
    ...
    equals(other: ProjectiveVector3DReal, tolerance?: number): boolean {
        return super.equals(other, tolerance);
    }

    isParallel(other: ProjectiveVector3DReal, angularTolerance?: number): boolean {
        return super.isParallel(other, angularTolerance);
    }

    isOrthogonal(other: ProjectiveVector3DReal, angularTolerance?: number): boolean {
        return super.isOrthogonal(other, angularTolerance);
    }
    
    toRealVector(vectorSpace?: RealVectorSpace<3>): Vector3DReal {
    ...
    createVectorFromDescriptor(descriptor: ProjectiveRealVector3D): this {
        return new ProjectiveVector3DReal(descriptor.coordinates[0], descriptor.coordinates[1], descriptor.coordinates[2], descriptor.coordinates[3].weight, this.vectorSpace) as this;
    }
}