...
export class OpPeriodicBSplineR1toR2Hessian extends BaseOpProblemBSplineR1toR2 {
    ...
    private Dh5xx: ScaledBernsteinDecompositionR1toR1[][] = []
    private Dh6_7xy: ScaledBernsteinDecompositionR1toR1[][] = []
    private Dh8_9xx: ScaledBernsteinDecompositionR1toR1[][] = []
    private Dh10_11xy: ScaledBernsteinDecompositionR1toR1[][] = []
    ...
    constructor(target: PeriodicBSplineR1toR2, initial: PeriodicBSplineR1toR2, public activeControl: ActiveControl = ActiveControl.curvatureExtrema) {
        ...
        this._hessian_f = this.compute_hessian_f(e, this.curvatureExtremaConstraintsSign, this._curvatureExtremaInactiveConstraints)
    ...
    compute_hessian_f( e: ExpensiveComputationResults,
        constraintsSign: number[], 
        inactiveConstraints: number[]) {
        ...
        let h5x: ScaledBernsteinDecompositionR1toR1[] = []
        let h5y: ScaledBernsteinDecompositionR1toR1[] = []         
        let h6x: ScaledBernsteinDecompositionR1toR1[] = []
        let h6y: ScaledBernsteinDecompositionR1toR1[] = []
        let h7x: ScaledBernsteinDecompositionR1toR1[] = []
        let h7y: ScaledBernsteinDecompositionR1toR1[] = []        
        let h8x: ScaledBernsteinDecompositionR1toR1[] = []
        let h8y: ScaledBernsteinDecompositionR1toR1[] = []
        let h9x: ScaledBernsteinDecompositionR1toR1[] = []
        let h9y: ScaledBernsteinDecompositionR1toR1[] = []
        let h10x: ScaledBernsteinDecompositionR1toR1[] = []
        let h10y: ScaledBernsteinDecompositionR1toR1[] = []        
        let h11x: ScaledBernsteinDecompositionR1toR1[] = []
        let h11y: ScaledBernsteinDecompositionR1toR1[] = []
        ...
        for (let i = 0; i < n; i += 1){
            h5x.push(this.dBasisFunctions_du[i].multiply(e.bdsxu))
            h6x.push(this.dBasisFunctions_du[i].multiply(e.bdsyuuu))
            h7x.push(e.bdsyu.multiply(this.d3BasisFunctions_du3[i]).multiplyByScalar(-1))
            h8x.push(this.dBasisFunctions_du[i].multiply(e.bdsxuu))
            h9x.push(e.bdsxu.multiply(this.d2BasisFunctions_du2[i]))
            h10x.push(this.dBasisFunctions_du[i].multiply(e.bdsyuu))
            h11x.push(e.bdsyu.multiply(this.d2BasisFunctions_du2[i]).multiplyByScalar(-1))
        }
        for (let i = 0; i < n; i += 1){
            h5y.push(this.dBasisFunctions_du[i].multiply(e.bdsyu))
            h6y.push(this.dBasisFunctions_du[i].multiply(e.bdsxuuu).multiplyByScalar(-1))
            h7y.push(e.bdsxu.multiply(this.d3BasisFunctions_du3[i]))
            h8y.push(this.dBasisFunctions_du[i].multiply(e.bdsyuu))
            h9y.push(e.bdsyu.multiply(this.d2BasisFunctions_du2[i]))
            h10y.push(this.dBasisFunctions_du[i].multiply(e.bdsxuu).multiplyByScalar(-1));
            h11y.push(e.bdsxu.multiply(this.d2BasisFunctions_du2[i]))
        }
        ...
        for (let i = 0; i < n; i += 1){
            for (let j = 0; j <= i; j += 1){
                const term1 = this.Dh5xx[i][j].multiply(e.cuXcuuu).multiplyByScalar(2)
                ...
                // term3 = 0
                const term4 = this.Dh8_9xx[i][j].multiply(e.cuXcuu).multiplyByScalar(-3)
        ...
        for (let i = 1; i < n; i += 1){
            for (let j = 0; j < i; j += 1){
                ...
                const term3 = this.Dh6_7xy[j][i].multiply(e.cuDOTcu).multiplyByScalar(-1) //Dh_6_7xy is antisymmetric
                // term4 = 0
                ...
                const term6 = this.Dh10_11xy[j][i].multiply(e.cuDOTcuu).multiplyByScalar(3); //Dh_10_11xy is antisymmetric
        ...
        for (let i = 0; i < n; i += 1){
            for (let j = i + 1; j < n; j += 1){
                // term1 = 0
                const term2xy = ((h5x[j].multiply((h6y[i].add(h7y[i])))).add(h5y[i].multiply((h6x[j].add(h7x[j]))))).multiplyByScalar(2)
                const term3 = this.Dh6_7xy[i][j].multiply(e.cuDOTcu) //Dh_6_7xy is antisymmetric
                // term4 = 0
                const term5xy = (((h8x[j].add(h9x[j])).multiply((h10y[i].add(h11y[i])))).add((h8y[i].add(h9y[i])).multiply((h10x[j].add(h11x[j]))))).multiplyByScalar(-3)
                const term6 = this.Dh10_11xy[i][j].multiply(e.cuDOTcuu).multiplyByScalar(-3); //Dh_10_11xy is antisymmetric
                hessian_gxy[i][j] = (term2xy.add(term3).add(term5xy).add(term6)).flattenControlPointsArray();
    ...
    prepareForHessianComputation(Dsu: ScaledBernsteinDecompositionR1toR1[], Dsuu: ScaledBernsteinDecompositionR1toR1[], Dsuuu: ScaledBernsteinDecompositionR1toR1[]) {
        const n = this.spline.freeControlPoints.length
