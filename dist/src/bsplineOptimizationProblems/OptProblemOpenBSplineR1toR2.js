...
const PolygonWithVerticesR1_1 = require("../containers/PolygonWithVerticesR1");
const OscillatingPolygonWithVerticesR1_1 = require("../containers/OscillatingPolygonWithVerticesR1");
...
const AbstractOptProblemBSplineR1toR2_1 = require("./AbstractOptProblemBSplineR1toR2");
const ShapeSpaceBoundaryEnforcer_1 = require("../curveShapeSpaceNavigation/ShapeSpaceBoundaryEnforcer");
var eventMove;
(function (eventMove) {
...
class OptProblemOpenBSplineR1toR2 extends AbstractOptProblemBSplineR1toR2_1.AbstractOptProblemBSplineR1toR2 {
    constructor(splineInitial, shapeSpaceDiffEventsStructure) {
    ...
    computeBasisFunctionsDerivatives() {
        ...
        let secondOrderSplineDerivatives = [];
        ...
        if (this._shapeSpaceDiffEventsStructure.activeControlInflections || this._shapeSpaceDiffEventsStructure.activeControlCurvatureExtrema) {
            for (let i = 0; i < n; i += 1) {
                ...
                let s = new BSplineR1toR1_1.BSplineR1toR1(diracControlPoints.slice(), this.spline.knots.slice());
                let su = s.derivative();
    ...
    computeSignChangeIntervals(constraintsSign) {
        let signChangesIntervals = [];
    ...
    inactivateConstraintsAtCurveEXtremities(controlPoints, inactiveConstraints) {
    ...
    extractVerticesLocallyClosestToZero(controlPoints) {
        let indicesConstraints = [];
        const polygon = new PolygonWithVerticesR1_1.PolygonWithVerticesR1(controlPoints);
        const oscillatingPolygons = polygon.extractOscillatingPolygons();
        if (oscillatingPolygons.length !== 0) {
            const oscillatingPolygonsWithAdjacency = (0, OscillatingPolygonWithVerticesR1_1.extractAdjacentOscillatingPolygons)(oscillatingPolygons);
    ...
    checkConstraintTypeConsistency(controlPoints) {
        let valid = false;
        ...
        if (!valid) {
            const error = new ErrorLoging_1.ErrorLog(this.constructor.name, "checkConstraintTypeConsistency", "The number of constraints to analyse is not consistent with the type of constraint prescribed: please check.");
            error.logMessage();
        }
    }
    g() {
    ...
    gradient_curvatureDerivativeNumerator(sxu, syu, sxuu, syuu, sxuuu, syuuu, h1, h2, h3, h4) {
        let dgx = [];
        let dgy = [];
        const m = this.spline.controlPoints.length;
        const n = this.curvatureExtremaTotalNumberOfConstraints;
        let result = new DenseMatrix_1.DenseMatrix(n, 2 * m);
        for (let i = 0; i < m; i += 1) {
    ...
    compute_curvatureExtremaConstraints_gradient(constraintsSign, inactiveConstraints) {
        // const sxu = e.bdsxu
        // const sxuu = e.bdsxuu
    ...
    compute_curvatureExtremaConstraints_gradientPreviousIteration(constraintsSign, inactiveConstraints) {
    ...
    traceCurvatureExtrema() {
        let curvatureExtrema_gradientsPrevious = [];
        ...
        const curveDP = new BSplineR1toR2DifferentialProperties_1.BSplineR1toR2DifferentialProperties(this.previousSpline);
        ...
        const curveDPcurrent = new BSplineR1toR2DifferentialProperties_1.BSplineR1toR2DifferentialProperties(this.spline);
        ...
        for (let i = 0; i < zerosCuratureDerivPrevious.length; i++) {
            ...
            for (let k = 0; k < gradient_curvatureExtremaPrevious.shape[1]; k++) {
                ...
                const spline = new BSplineR1toR1_1.BSplineR1toR1(gradient, knotsCurvatureDerivNumerator);
    ...
    //                                         constraintsSign: number[], 
    //                                         inactiveConstraints: number[]): DenseMatrix {
    compute_inflectionConstraints_gradient(constraintsSign, inactiveConstraints) {
        // const sxu = e.bdsxu
        // const sxuu = e.bdsxuu
        // const syu = e.bdsyu
        // const syuu = e.bdsyuu
    ...
    compute_inflectionConstraints_gradientPreviousIteration(constraintsSign, inactiveConstraints) {
        const sxu = this._previousAnalyticHighOrderCurveDerivatives.bdsxu;
        const sxuu = this._previousAnalyticHighOrderCurveDerivatives.bdsxuu;
        const syu = this._previousAnalyticHighOrderCurveDerivatives.bdsyu;
        const syuu = this._previousAnalyticHighOrderCurveDerivatives.bdsyuu;
    ...
    compute_hessian_f(sxu, syu, sxuu, syuu, sxuuu, syuuu, h1, h2, h3, h4, constraintsSign, inactiveConstraints) {
        const n = this.spline.controlPoints.length;
        let result = [];
        let h5x = [];
        let h5y = [];
        ...
        for (let k = 0; k < constraintsSign.length; k += 1) {
            if (k === inactiveConstraints[deltak]) {
                ...
                let m = new SymmetricMatrix_1.SymmetricMatrix(2 * n);
                for (let i = 0; i < n; i += 1) {
                    for (let j = 0; j <= i; j += 1) {
    ...
    prepareForHessianComputation(Dsu, Dsuu, Dsuuu) {
        const n = this.spline.controlPoints.length;
        for (let i = 0; i < n; i += 1) {
            this.Dh5xx.push([]);
            this.Dh6_7xy.push([]);
...
class OptProblemOPenBSplineR1toR2WithWeigthingFactors extends OptProblemOpenBSplineR1toR2 {
    constructor(splineInitial, shapeSpaceDiffEventsStructure) {
        super(splineInitial, shapeSpaceDiffEventsStructure);
        this.weigthingFactors = [];
        for (let i = 0; i < this.spline.controlPoints.length * 2; i += 1) {
            this.weigthingFactors.push(exports.DEFAULT_WEIGHT);
        }
    }
    get f0() {
        let result = 0;
...
class OptProblemOPenBSplineR1toR2WithWeigthingFactorsEventMonitoringAtExtremities extends OptProblemOPenBSplineR1toR2WithWeigthingFactors {
    constructor(splineInitial, shapeSpaceDiffEventsStructure) {
        super(splineInitial, shapeSpaceDiffEventsStructure);
        this._shapeSpaceBoundaryEnforcer = new ShapeSpaceBoundaryEnforcer_1.NestedShapeSpacesBoundaryEnforcerOpenCurve();
        let e = this.initExpansiveComputations();
        e = this.expensiveComputation(this._spline);
        if (this._shapeSpaceDiffEventsStructure.activeControlInflections || this._shapeSpaceDiffEventsStructure.activeControlCurvatureExtrema) {
            this.constraintType = AbstractOptProblemBSplineR1toR2_1.ConstraintType.inflection;
            this._inflectionInactiveConstraints = this.computeInactiveConstraints(this._curvatureNumeratorCP);
    ...
    get shapeSpaceBoundaryEnforcer() {
    ...
    step(deltaX) {
        ...
        // let e: ExpensiveComputationResults = this.initExpansiveComputations();
        this._analyticHighOrderCurveDerivatives = this.initExpansiveComputations();
        let curvatureNumerator = [];
        let curvatureDerivativeNumerator = [];
...
class OptProblemOpenBSplineR1toR2WithWeigthingFactorsNoInactiveConstraints extends OptProblemOPenBSplineR1toR2WithWeigthingFactors {
    constructor(splineInitial, shapeSpaceDiffEventsStructure) {
        super(splineInitial, shapeSpaceDiffEventsStructure);
    }
...
class OptProblemOpenBSplineR1toR2NoInactiveConstraints extends OptProblemOpenBSplineR1toR2 {
    constructor(splineInitial, shapeSpaceDiffEventsStructure) {
        super(splineInitial, shapeSpaceDiffEventsStructure);
    }
    computeInactiveConstraints(curvatureDerivativeNumerator) {
...
class OptProblemOpenBSplineR1toR2WithWeigthingFactorsDedicatedToCubics extends OptProblemOPenBSplineR1toR2WithWeigthingFactors {
    constructor(splineInitial, shapeSpaceDiffEventsStructure) {
        super(splineInitial, shapeSpaceDiffEventsStructure);
    }
    computeControlPointsClosestToZeroForCubics(signChangesIntervals, controlPoints) {
        let result = [];
    ...
    addInactiveConstraintsForInflections(list, controlPoints) {
        let result = [];
...
class OptProblemOpenBSplineR1toR2WithWeigthingFactorsStrictShapeSpace extends OptProblemOPenBSplineR1toR2WithWeigthingFactors {
    constructor(splineInitial, shapeSpaceDiffEventsStructure, navigationCurveModel) {
        super(splineInitial, shapeSpaceDiffEventsStructure);
        ...
        this._diffEventsVariation = new DifferentialEventVariation_1.DiffrentialEventVariation(this.curveAnalyzerCurrentCurve, this.curveAnalyzerOptimizedCurve);
        this._shapeSpaceBoundaryEnforcer = new ShapeSpaceBoundaryEnforcer_1.StrictShapeSpacesBoundaryEnforcerOpenCurve();
        ...
        this.inflectionsConstraintsBounds = (0, MathVectorBasicOperations_1.zeroVector)(this._curvatureNumeratorCP.length);
        if (this._shapeSpaceDiffEventsStructure.activeControlCurvatureExtrema) {
            this.constraintType = AbstractOptProblemBSplineR1toR2_1.ConstraintType.curvatureExtrema;
            this._curvatureExtremaInactiveConstraints = this.computeInactiveConstraints(this._curvatureDerivativeNumeratorCP);
            this.curvatureExtremaNumberOfActiveConstraints = this._curvatureDerivativeNumeratorCP.length - this.curvatureExtremaInactiveConstraints.length;
    ...
    set diffEventsVariation(diffEventsVariation) {
    ...
    checkConstraintConsistency() {
        ...
        for (let i = 0; i < this._f.length; i += 1) {
            if (this._f[i] > 0.0) {
                ...
                if (this._shapeSpaceDiffEventsStructure.activeControlInflections && this._shapeSpaceDiffEventsStructure.activeControlCurvatureExtrema) {
                    ...
                    const warning = new ErrorLoging_1.WarningLog(this.constructor.name, "checkConstraintConsistency", "No active control set. There should be no constraint.");
                    warning.logMessage();
                }
        ...
        if (invalidConstraints.length > 0) {
            ...
            const error = new ErrorLoging_1.ErrorLog(this.constructor.name, "checkConstraintConsistency", message);
            error.logMessage();
        }
    }
    inactivateConstraintClosestToZero(controlPoints, inactiveConstraints) {
        const polygonOfCtrlPts = new PolygonWithVerticesR1_1.PolygonWithVerticesR1(controlPoints);
    ...
    // }
    inactivateConstraintsWithinRangeOfLocalExtremum(inactiveConstraints) {
    ...
    clearInequalityChanges() {
        if (this.constraintType === AbstractOptProblemBSplineR1toR2_1.ConstraintType.curvatureExtrema) {
            ...
            const error = new ErrorLoging_1.ErrorLog(this.constructor.name, "clearInequalityChanges", "Current constraint type is not compatible with the inequalities changes.");
            error.logMessage();
        }
    }
    clearConstraintBoundsUpdate() {
        if (this.constraintType === AbstractOptProblemBSplineR1toR2_1.ConstraintType.curvatureExtrema) {
            for (let controlPoint of this._curvatureDerivativeNumeratorCP) {
            ...
            const error = new ErrorLoging_1.ErrorLog(this.constructor.name, "clearConstraintBoundsUpdate", "Current constraint type is not compatible with the constraint bounds update.");
            error.logMessage();
        }
    }
    revertInequalitiesWithinRangeOfLocalExtremum() {
    ...
    updateConstraintBoundsWithinRangeOfLocalExtremum() {
        if (this._diffEventsVariation.neighboringEvents.length === 0) {
            return;
        }
        else if ((this._diffEventsVariation.neighboringEvents[0].type === NeighboringEvents_1.NeighboringEventsType.neighboringCurvatureExtremaDisappear
    ...
    compute_gradient_f(inflectionConstraintsSign, inflectionInactiveConstraints, curvatureExtremaConstraintsSign, curvatureExtremaInactiveConstraints) {
        if (this._shapeSpaceDiffEventsStructure.activeControlInflections && this._shapeSpaceDiffEventsStructure.activeControlCurvatureExtrema) {
            ...
            const warning = new ErrorLoging_1.WarningLog(this.constructor.name, "compute_gradient_f", "active control set to none: unable to compute gradients of f.");
            warning.logMessage();
            let result = new DenseMatrix_1.DenseMatrix(1, 1);
            return result;
        }
    }
    step(deltaX) {
        ...
        let curvatureNumerator = [];
        let curvatureDerivativeNumerator = [];
        let curvatureDerivativeEXtrema = [];
        let curvatureDerivativeEXtremaUpdated = [];
        if (this._shapeSpaceBoundaryEnforcer.isActive()) {
            this._inflectionInactiveConstraints = [];
            this._curvatureExtremaInactiveConstraints = [];
        }
        ...
        if (this.diffEventsVariation.neighboringEvents.length > 0) {
            if (this._diffEventsVariation.neighboringEvents[0].type === NeighboringEvents_1.NeighboringEventsType.neighboringCurvatureExtremaDisappear || this._diffEventsVariation.neighboringEvents[0].type === NeighboringEvents_1.NeighboringEventsType.neighboringCurvatureExtremaAppear) {
                const splineDP = new BSplineR1toR2DifferentialProperties_1.BSplineR1toR2DifferentialProperties(this.spline);
                ...
                this._spline = this.spline.moveControlPoints((0, AbstractOptProblemBSplineR1toR2_1.convertStepToVector2d)(deltaX));
                const splineDPupdated = new BSplineR1toR2DifferentialProperties_1.BSplineR1toR2DifferentialProperties(this.spline);
    ...
    cancelEvent() {
        ...
        let delta = (0, MathVectorBasicOperations_1.zeroVector)(this.spline.controlPoints.length * 2);
        this.step(delta);
        this.checkConstraintConsistency();
    }
}
