...
const MathVectorBasicOperations_4 = require("../linearAlgebra/MathVectorBasicOperations");
const MathVectorBasicOperations_5 = require("../linearAlgebra/MathVectorBasicOperations");
const MathVectorBasicOperations_6 = require("../linearAlgebra/MathVectorBasicOperations");
const MathVectorBasicOperations_7 = require("../linearAlgebra/MathVectorBasicOperations");
const MathVectorBasicOperations_8 = require("../linearAlgebra/MathVectorBasicOperations");
const MathVectorBasicOperations_9 = require("../linearAlgebra/MathVectorBasicOperations");
...
class TrustRegionSubproblem {
    ...
    update_lambda_lambdaPlus_lowerBound_and_step(trustRegionRadius, hessianPlusLambda, choleskyDecomposition) {
        ...
        if (this.range === lambdaRange.L || this.range === lambdaRange.G) {
            ...
            let wSquaredNorm = (0, MathVectorBasicOperations_3.squaredNorm)(w);
            ...
            let vSquaredNorm = (0, MathVectorBasicOperations_3.squaredNorm)(sls.vector);
    ...
    check_for_termination_and_update_step(trustRegionRadius, hessianPlusLambda, choleskyDecomposition) {
        ...
        if ((this.range === lambdaRange.L || this.range === lambdaRange.G) && Math.abs(this.stepNorm - trustRegionRadius) <= this.k_easy * trustRegionRadius) {
            ...
            let evalResult = (0, MathVectorBasicOperations_6.dotProduct)(this.gradient, this.step) + 0.5 * this.hessian.quadraticForm(this.step);
            let evalCauchy = (0, MathVectorBasicOperations_6.dotProduct)(this.gradient, this.cauchyPoint) + 0.5 * this.hessian.quadraticForm(this.cauchyPoint);
    ...
    computeCauchyPoint(trustRegionRadius) {
        ...
        let result = (0, MathVectorBasicOperations_4.multiplyVectorByScalar)(this.gradient, -trustRegionRadius / gNorm);
        if (gHg <= 0) {
            return result;
        }
...
function singularLeadingSubmatrix(A, L, k) {
    ...
    let v = [];
    let u = (0, MathVectorBasicOperations_8.zeroVector)(k);
...
function estimateSmallestSingularValue(lowerTriangular) {
    ...
    let p = (0, MathVectorBasicOperations_8.zeroVector)(n);
    let y = (0, MathVectorBasicOperations_8.zeroVector)(n);
    let p_plus = [];
    let p_minus = [];
    for (let k = 0; k < n; k += 1) {
...
function getBoundariesIntersections(z, d, trustRegionRadius) {
    ...
    const a = (0, MathVectorBasicOperations_3.squaredNorm)(d);
    const b = 2 * (0, MathVectorBasicOperations_6.dotProduct)(z, d);
    const c = (0, MathVectorBasicOperations_3.squaredNorm)(z) - trustRegionRadius * trustRegionRadius;
    const sqrtDiscriminant = Math.sqrt(b * b - 4 * a * c);
    let sign_b = (0, MathVectorBasicOperations_9.sign)(b);
    if (sign_b === 0) {
        sign_b = 1;
    }
