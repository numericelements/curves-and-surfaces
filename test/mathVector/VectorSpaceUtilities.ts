...
import { areSameVSpaceAndDimension, isComplexVector, isProjectiveComplexVector, isProjectiveRealVector, isRealVector, isVector1D, isVector2D, isVector3D, isVector4D, sendRangeErrorMessage } from "../../src/mathVector/VectorSpaceUtilities";
...
import { RealVector1D, VectorDesc1D, VectorDesc2D, VectorDesc3D, VectorDesc4D } from "../../src/mathVector/utilityTypes/VectorDescriptorTypes";

describe('isVector1D', () => {

    it('checks that a Vector1d is effectively of type Real or RealVector1D or ' + COMPLEX, () => {
        const vec1: VectorDesc1D = 0;
        ...
        const vec2: VectorDesc1D = {type: COMPLEX, real: 0, imaginary: 0};
        const res2 = isVector1D(vec2)
        expect(res2).to.eql(true)
    });

    it('checks that a Vector describing a Vector2D is not of type Vector1D', () => {
        const vec1: VectorDesc2D = {type: REALVECTOR2D, coordinates: [0, 0]};
        const res1 = isVector1D(vec1)
        expect(res1).to.eql(false)

        const vec2: VectorDesc2D = {type: COMPLEXVECTOR2D, coordinates: [{type: COMPLEX, real: 0, imaginary: 0}, {type: COMPLEX, real: 0, imaginary: 0}]};
        const res2 = isVector1D(vec2)
        expect(res2).to.eql(false)

        const vec3: VectorDesc2D = {type: PROJECTIVECOMPLEXVECTOR1D, coordinates: [{type: COMPLEX, real: 0, imaginary: 0}, {type: COMPLEXWEIGHT, real: new Weight(1), imaginary: new Weight(1)}]};
        const res3 = isVector1D(vec3)
        expect(res3).to.eql(false)
    });

    it('checks that a Vector describing a Vector3D is not of type Vector1D', () => {
        const vec1: VectorDesc3D = {type: REALVECTOR3D, coordinates: [0, 0, 0]};
        const res1 = isVector1D(vec1)
        expect(res1).to.eql(false)

        const vec3: VectorDesc3D = {type: PROJECTIVEREALVECTOR2D, coordinates: [0, 0, {type: WEIGHT, weight: new Weight(1)}]};
        const res3 = isVector1D(vec3)
        expect(res3).to.eql(false)
    });

    it('checks that a Vector describing a Vector4D is not of type Vector1D', () => {
        const vec1: VectorDesc4D = {type: REALVECTOR4D, coordinates: [0, 0, 0, 0]};
        const res1 = isVector1D(vec1)
        expect(res1).to.eql(false)

        const vec3: VectorDesc4D = {type: PROJECTIVEREALVECTOR3D, coordinates: [0, 0, 0, {type: WEIGHT, weight: new Weight(1)}]};
        const res3 = isVector1D(vec3)
        expect(res3).to.eql(false)
    });
...
describe('isVector2D', () => {
    ...
    it('checks that a Vector2D is effectively of type RealVector2D or ComplexVector2D or ProjectiveComplexVector1D', () => {
        const vec3: VectorDesc2D = {type: REALVECTOR2D, coordinates: [0, 0]};
        ...
        const vec1: VectorDesc2D = {type: PROJECTIVECOMPLEXVECTOR1D, coordinates: [{type: COMPLEX, real: 0, imaginary: 0}, {type: COMPLEXWEIGHT, real: new Weight(1), imaginary: new Weight(1)}]};
...
describe('isVector3D', () => {
    ...
    it('checks that a Vector3D is effectively of type RealVector3D or ProjectiveRealVector2D', () => {
        const vec3: VectorDesc3D = {type: REALVECTOR3D, coordinates: [0, 0, 0]};
        const res3 = isVector3D(vec3)
        expect(res3).to.eql(true)

        const vec1: VectorDesc3D = {type: PROJECTIVEREALVECTOR2D, coordinates: [0, 0, {type: WEIGHT, weight: new Weight(1)}]};
    ...
    it('checks that a Vector describing a Vector2D is not of type Vector3D', () => {
        ...
        const vec4: VectorDesc2D = {type: PROJECTIVECOMPLEXVECTOR1D, coordinates: [{type: COMPLEX, real: 0, imaginary: 0}, {type: COMPLEXWEIGHT, real: new Weight(1), imaginary: new Weight(1)}]};
...
describe('isVector4D', () => {
    ...
    it('checks that a Vector4D is effectively of type RealVector4D or ProjectiveRealVector3D', () => {
        const vec3: VectorDesc4D = {type: REALVECTOR4D, coordinates: [0, 0, 0, 0]};
        const res3 = isVector4D(vec3)
        expect(res3).to.eql(true)

        const vec1: VectorDesc4D = {type: PROJECTIVEREALVECTOR3D, coordinates: [0, 0, 0, {type: WEIGHT, weight: new Weight(1)}]};
...
describe('isRealVector', () => {
    ...
    it('checks that a RealVector is effectively of type Real or ' + REALVECTOR2D + ' or ' + REALVECTOR3D + ' or ' + REALVECTOR4D, () => {
        ...
        const vec2: VectorDesc3D = {type: REALVECTOR3D, coordinates: [0, 0, 0]};
        const res1 = isRealVector(vec2)
        expect(res1).to.eql(true)

        const vec4: VectorDesc4D = {type: REALVECTOR4D, coordinates: [0, 0, 0, 0]};
    ...
    it('checks that a Vector describing a ComplexVector is not of type RealVector', () => {
        const vec1: VectorDesc1D = {type: COMPLEX, real: 0, imaginary: 0};
    ...
    it('checks that a Vector describing a ProjectiveRealVector is not of type RealVector', () => {
        const vec3: VectorDesc3D = {type: PROJECTIVEREALVECTOR2D, coordinates: [0, 0, {type: WEIGHT, weight: new Weight(1)}]};
        const res3 = isRealVector(vec3)
        expect(res3).to.eql(false)
...
describe('isComplexVector', () => {
    ...
    it('checks that a Vector describing a ProjectiveRealVector is not of type ComplexVector', () => {
        const vec3: VectorDesc3D = {type: PROJECTIVEREALVECTOR2D, coordinates: [0, 0, {type: WEIGHT, weight: new Weight(1)}]};
        const res3 = isComplexVector(vec3)
        expect(res3).to.eql(false)
...
describe('isProjectiveRealVector', () => {

    it('checks that a ProjectiveRealVector is effectively of type ' + PROJECTIVEREALVECTOR2D + ' or ' + PROJECTIVEREALVECTOR3D, () => {
        const vec3: VectorDesc3D = {type: PROJECTIVEREALVECTOR2D, coordinates: [0, 0, {type: WEIGHT, weight: new Weight(1)}]};
        const res3 = isProjectiveRealVector(vec3)
        expect(res3).to.eql(true)

        const vec1: VectorDesc4D = {type: PROJECTIVEREALVECTOR3D, coordinates: [0, 0, 0, {type: WEIGHT, weight: new Weight(1)}]};
        const res1 = isProjectiveRealVector(vec1)
        expect(res1).to.eql(true)
    });

    it('checks that a Vector describing a RealVector is not of type ProjectiveRealVector', () => {
        const vec1: VectorDesc1D = 0;
        const res = isProjectiveRealVector(vec1)
        expect(res).to.eql(false)
        ...
        const vec2: VectorDesc3D = {type: REALVECTOR3D, coordinates: [0, 0, 0]};
        const res1 = isProjectiveRealVector(vec2)
        expect(res1).to.eql(false)

        const vec4: VectorDesc4D = {type: REALVECTOR4D, coordinates: [0, 0, 0, 0]};
        const res4 = isProjectiveRealVector(vec4)
        expect(res4).to.eql(false)
    });

    it('checks that a Vector describing a ComplexVector is not of type ProjectiveRealVector', () => {
        const vec1: VectorDesc1D = {type: COMPLEX, real: 0, imaginary: 0};
        const res1 = isProjectiveRealVector(vec1)
        expect(res1).to.eql(false)

        const vec2: VectorDesc2D = {type: COMPLEXVECTOR2D, coordinates: [{type: COMPLEX, real: 0, imaginary: 0}, {type: COMPLEX, real: 0, imaginary: 0}]};
        const res2 = isProjectiveRealVector(vec2)
        expect(res2).to.eql(false)
    });

    it('checks that a Vector describing a ProjectiveComplexVector is not of type ProjectiveRealVector', () => {
        const vec4: VectorDesc2D = {type: PROJECTIVECOMPLEXVECTOR1D, coordinates: [{type: COMPLEX, real: 0, imaginary: 0}, {type: COMPLEXWEIGHT, real: new Weight(1), imaginary: new Weight(1)}]};
        const res1 = isProjectiveRealVector(vec4)
        expect(res1).to.eql(false)
    });
});
...
describe('isProjectiveComplexVector', () => {
    ...
    it('checks that a Vector describing a ProjectiveRealVector is not of type ProjectiveComplexVector', () => {
        const vec3: VectorDesc3D = {type: PROJECTIVEREALVECTOR2D, coordinates: [0, 0, {type: WEIGHT, weight: new Weight(1)}]};
        const res3 = isProjectiveComplexVector(vec3)
        expect(res3).to.eql(false)
...
describe('areSameVSpaceAndDimension', () => {
    ...
    it('case of RealVectors of same dimesions ' + MIN_DIMENSION_REALVECTORSPACE + ' through ' + MAX_DIMENSION_REALVECTORSPACE, () => {
        const vec1: VectorDesc1D = 0;
        const vec2: VectorDesc1D = 1;
        const res = areSameVSpaceAndDimension(vec1, vec2);
        expect(res).to.eql(true)

        const vec3: VectorDesc2D = {type: REALVECTOR2D, coordinates: [0, 0]};
        const vec4: VectorDesc2D = {type: REALVECTOR2D, coordinates: [0, 1]};
        const res1 = areSameVSpaceAndDimension(vec3, vec4)
        expect(res1).to.eql(true)

        const vec5: VectorDesc3D = {type: REALVECTOR3D, coordinates: [0, 0, 0]};
        const vec6: VectorDesc3D = {type: REALVECTOR3D, coordinates: [0, 1, 0]};
        const res2 = areSameVSpaceAndDimension(vec5, vec6)
        expect(res2).to.eql(true)

        const vec7: VectorDesc4D = {type: REALVECTOR4D, coordinates: [0, 0, 0, 0]};
        const vec8: VectorDesc4D = {type: REALVECTOR4D, coordinates: [0, 1, 0, 0]};
    ...
    it('case of ComplexVectors of same dimesions ' + MIN_DIMENSION_COMPLEXVECTORSPACE + ' through ' + MAX_DIMENSION_COMPLEXVECTORSPACE, () => {
        const vec1: VectorDesc1D = {type: COMPLEX, real: 0, imaginary: 0};
        const vec2: VectorDesc1D = {type: COMPLEX, real: 1, imaginary: 0};
        const res = areSameVSpaceAndDimension(vec1, vec2);
        expect(res).to.eql(true)

        const vec3: VectorDesc2D = {type: COMPLEXVECTOR2D, coordinates: [{type: COMPLEX, real: 0, imaginary: 0}, {type: COMPLEX, real: 0, imaginary: 0}]};
        const vec4: VectorDesc2D = {type: COMPLEXVECTOR2D, coordinates: [{type: COMPLEX, real: 1, imaginary: 0}, {type: COMPLEX, real: 0, imaginary: 1}]};
        const res1 = areSameVSpaceAndDimension(vec3, vec4)
        expect(res1).to.eql(true)
    });
    ...
    it('case of ProjectiveRealVectors of same dimesions ' + MIN_DIMENSION_PROJECTIVEREALVECTORSPACE + ' through ' + MAX_DIMENSION_PROJECTIVEREALVECTORSPACE, () => {
        const vec1: VectorDesc3D = {type: PROJECTIVEREALVECTOR2D, coordinates: [0, 0, {type: WEIGHT, weight: new Weight(1)}]};
        const vec2: VectorDesc3D = {type: PROJECTIVEREALVECTOR2D, coordinates: [1, 0, {type: WEIGHT, weight: new Weight(1)}]};
        const res = areSameVSpaceAndDimension(vec1, vec2);
        expect(res).to.eql(true)

        const vec3: VectorDesc4D = {type: PROJECTIVEREALVECTOR3D, coordinates: [0, 0, 0, {type: WEIGHT, weight: new Weight(1)}]};
        const vec4: VectorDesc4D = {type: PROJECTIVEREALVECTOR3D, coordinates: [0, 0, 0, {type: WEIGHT, weight: new Weight(1)}]};
        const res1 = areSameVSpaceAndDimension(vec3, vec4);
        expect(res1).to.eql(true)
    });

    it('case of ProjectiveRealVectors of different dimesions ranging from ' + MIN_DIMENSION_PROJECTIVEREALVECTORSPACE + ' through ' + MAX_DIMENSION_PROJECTIVEREALVECTORSPACE, () => {
        const vec1: VectorDesc3D = {type: PROJECTIVEREALVECTOR2D, coordinates: [0, 0, {type: WEIGHT, weight: new Weight(1)}]};
        const vec3: VectorDesc4D = {type: PROJECTIVEREALVECTOR3D, coordinates: [0, 0, 0, {type: WEIGHT, weight: new Weight(1)}]};
    ...
    it('case of ProjectiveComplexVectors of same dimesions ' + MIN_DIMENSION_PROJECTIVECOMPLEXVECTORSPACE + ' through ' + MAX_DIMENSION_PROJECTIVECOMPLEXVECTORSPACE, () => {
        const vec1: VectorDesc2D = {type: PROJECTIVECOMPLEXVECTOR1D, coordinates: [{type: COMPLEX, real: 0, imaginary: 0}, {type: COMPLEXWEIGHT, real: new Weight(1), imaginary: new Weight(1)}]};
        const vec2: VectorDesc2D = {type: PROJECTIVECOMPLEXVECTOR1D, coordinates: [{type: COMPLEX, real: 1, imaginary: 0}, {type: COMPLEXWEIGHT, real: new Weight(1), imaginary: new Weight(1)}]};
        const res = areSameVSpaceAndDimension(vec1, vec2);
        expect(res).to.eql(true)
    });
});