...
export function createCommonProjectiveVectorSpaceTests(
    ...
    describe('Common ProjectiveVector Space Tests', () => {
        ...
        it(`can scale a ${vectorType} vector with a real strictly positive value producing a weight smaller than ${NULL_WEIGHT_TOLERANCE} tolerance with weight management ${WeightManagement.AllPositiveWeights}`, () => {
            const vectorSpace = createProjectiveVectorSpace(WeightManagement.AllPositiveWeights);
            expect(vectorSpace.weightManagement).to.eql(WeightManagement.AllPositiveWeights);
        ...
        it(`cannot scale a ${vectorType} vector with a strictly negative value with weight management ${WeightManagement.AllPositiveWeights}`, () => {
            const vectorSpace = createProjectiveVectorSpace(WeightManagement.AllPositiveWeights);
            expect(vectorSpace.weightManagement).to.eql(WeightManagement.AllPositiveWeights);
        ...
        it(`can scale a ${vectorType} vector with a real, strictly positive value, producing a weight smaller than ${NULL_WEIGHT_TOLERANCE} tolerance with weight management ${WeightManagement.SomeNullWeights}`, () => {
            const vectorSpace = createProjectiveVectorSpace(WeightManagement.SomeNullWeights);
            expect(vectorSpace.weightManagement).to.eql(WeightManagement.SomeNullWeights);
            const vec1 = createTestProjectiveVector(vectorType, DEFAULT_WEIGHT_VALUE, true);
            let scale = NULL_WEIGHT_TOLERANCE / 3;
        ...
        it(`cannot scale a ${vectorType} vector with a strictly negative value with weight management ${WeightManagement.SomeNullWeights}`, () => {
            const vectorSpace = createProjectiveVectorSpace(WeightManagement.SomeNullWeights);
            expect(vectorSpace.weightManagement).to.eql(WeightManagement.SomeNullWeights);
            const vec1 = createTestProjectiveVector(vectorType, DEFAULT_WEIGHT_VALUE, false);
            const scale = -2;
