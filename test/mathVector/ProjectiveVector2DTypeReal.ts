...
import { ANGULAR_TOL_VECTOR, EM_NORM_TOO_SMALL, EM_PROJECTIVE_VECTOR_WEIGHT_STATUS_INCOMPATIBLE, EM_VECTORS_DIFFERENT_DIM, EM_VECTORS_NOT_IN_SAME_VECTORSPACE, LINEAR_TOL_VECTOR } from "../../src/namedConstants/Vectors";
...
import { EM_WEIGHT_STATUS_INCOMPATIBLE_STRICTLY_POSITIVE_MANAGEMENT } from "../../src/ErrorMessages/WeightManager";
...
describe('Projective vector 2D in real vector space: generation and operators in this vector space', () => {
    ...
    describe('Methods', () => {
        it(`cannot add a vector with another vector of different dimension`, () => {
            ...
            const projectiveVector1 = new ProjectiveVector2DTypeReal(coordinates[0], coordinates[1]);
            ...
            expect(projectiveVector1.vectorSpace.isDefault).to.eql(true);
            const projectiveVector2 = new ProjectiveVector3DTypeReal(coordinates[0], coordinates[1], coordinates[2]);
            expect(projectiveVector2.dimension).to.not.eql(projectiveVector1.dimension);
            expect(() => projectiveVector1.add(projectiveVector2)).to.throw(EM_VECTORS_DIFFERENT_DIM);
        });
        ...
        it(`cannot subtract a vector from another vector of different dimension`, () => {
            ...
            const projectiveVector2 = new ProjectiveVector3DTypeReal(coordinates[0], coordinates[1], coordinates[2]);
            expect(projectiveVector2.dimension).to.not.eql(projectiveVector1.dimension);
            expect(() => projectiveVector1.subtract(projectiveVector2)).to.throw(EM_VECTORS_DIFFERENT_DIM);
        });
        ...
        it(`can get the vector descriptor as a string`, () => {
            ...
            expect(string).to.eql(PROJECTIVEVECTOR2D + `(${coordinates[0]}, ${coordinates[1]}, ${new Weight().toString()})`);
        });

        it(`cannot check the equality of vectors of different dimensions `, () => {
            const coordinates = [1, 3];
            const vSpace = new ProjectiveVectorSpace(dimension);
            const projectiveVector1 = new ProjectiveVector2DTypeReal(coordinates[0], coordinates[1], vSpace);
            expect(projectiveVector1.dimension).to.eql(dimension);
            expect(projectiveVector1.vectorType).to.eql(PROJECTIVEVECTOR2D);
            expect(projectiveVector1.spaceType).to.eql(VectorSpaceType.PROJECTIVE);
            expect(projectiveVector1.vectorSpace.isDefault).to.eql(false);
            const vSpace1 = new ProjectiveVectorSpace(MAX_DIMENSION_PROJECTIVEVECTORSPACE);
            const projectiveVector2 = new ProjectiveVector3DTypeReal(coordinates[0], coordinates[1], 0, new Weight(), vSpace1);
            expect(() => projectiveVector1.equals(projectiveVector2)).to.throw(EM_PROJECTIVEVECTORS_DIFFERENT_DIM);
        });
        ...
        it(`cannot check the parallelism of vectors belonging to different vector spaces of different dimensions`, () => {
            ...
            expect(projectiveVector1.vectorSpace.isDefault).to.eql(false);
            const vSpace1 = new ProjectiveVectorSpace(MAX_DIMENSION_PROJECTIVEVECTORSPACE);
            const realVector2 = new ProjectiveVector3DTypeReal(coordinates[0], coordinates[1], 0, new Weight(), vSpace1);
