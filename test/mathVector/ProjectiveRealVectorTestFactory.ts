...
export function createCommonProjectiveRealVectorTests<D extends 3 | 4>(
    dimension: D
    // dimension: number
) {
    beforeEach(() => {
    ...
    describe('Common ProjectiveRealVector Tests', () => {
        ...
        describe('Accessors', () => {
            ...
            it(`can get the space dimension of a vector when it lies into a user-defined vector space`, () => {
                ...
                const projRealVector = createTestProjectiveRealVector(dimension, vSpace);
            ...
            it(`can get the space dimension of a vector when it lies into the default vector space`, () => {
                const projRealVector = createTestProjectiveRealVector(dimension);
            ...
            it(`can get the coordinates of a vector as an array of numbers`, () => {
                ...
                const projRealVector = createTestProjectiveRealVector(dimension, vSpace, coordinates, new Weight(coordinates[dimension - 1]));
            ...
            it(`can get the descriptor of a vector as vector coordinates`, () => {
                ...
                const weight = projRealVector.descriptor.coordinates[projRealVector.descriptor.coordinates.length - 1] as WeightDesc;
            ...
            it(`can get the homogeneous coordinates of a vector as an array of numbers`, () => {
                const coordinates = [-1, 0, 1, 2];
                const projRealVector = createTestProjectiveRealVector(dimension, undefined, coordinates, new Weight(coordinates[dimension - 1]));
            ...
            it(`can get the weight of a vector as a Weight in a default vector space with weight management ` + WeightManagement.AllStrictlyPositiveWeights, () => {
                ...
                const projRealVector = createTestProjectiveRealVector(dimension, undefined, coordinates, weight);
                expect(projRealVector.vectorSpace.isDefault).to.eql(true);
            ...
            it(`can get the weight of a vector as a Weight in a default vector space with weight management ` + WeightManagement.AllPositiveWeights, () => {
                ...
                const projRealVector = createTestProjectiveRealVector(dimension, vSpace, coordinates, weight);
                expect(projRealVector.vectorSpace.isDefault).to.eql(true);
            ...
            it(`can get the weight of a vector as a Weight in a default vector space with weight management ` + WeightManagement.SomeNullWeights, () => {
                ...
                const projRealVector1 = createTestProjectiveRealVector(dimension, vSpace, coordinates, weight1);
        ...
        describe('Methods', () => {
            ...
            it(`cannot get a coordinate of a vector when the coordinate index is negative`, () => {
                const coordinates = [1, 3, 5, 7];
                const vSpace = new ProjectiveRealVectorSpace(dimension);
                const projRealVector = createTestProjectiveRealVector(dimension, vSpace, coordinates);
                expect(projRealVector.dimension).to.eql(dimension);
                expect(projRealVector.spaceType).to.eql(VectorSpaceType.PROJECTIVEREAL);
                expect(projRealVector.vectorSpace.isDefault).to.eql(false);
            ...
            it(`can clone a vector living into a user-specified vector space`, () => {
                ...
                expect(newProjRealVector.spaceType).to.eql(VectorSpaceType.PROJECTIVEREAL);
            ...
            it(`can clone a vector living into a user-specified vector space with ${WeightManagement.AllPositiveWeights} and a null weight`, () => {
                ...
                const projRealVector = createTestProjectiveRealVector(dimension, vSpace, coordinates, nullWeight);
            ...
            it(`can compute the norm of a vector with weight management ${WeightManagement.AllStrictlyPositiveWeights}`, () => {
                ...
                const projRealVector1 = createTestProjectiveRealVector(dimension, vSpace, coordinates, new Weight(coordinates[dimension - 1]));
            ...
            it(`can compute the norm of a vector with weight management ${WeightManagement.AllPositiveWeights}`, () => {
                const coordinates = [2, 4, 6, 8];
                const vSpace = new ProjectiveRealVectorSpace(dimension, WeightManagement.AllPositiveWeights);
                const projRealVector1 = createTestProjectiveRealVector(dimension, vSpace, coordinates, new Weight(coordinates[dimension - 1], false));
                expect(projRealVector1.dimension).to.eql(dimension);
                expect(projRealVector1.vectorSpace.isDefault).to.eql(false);
            ...
            it(`cannot apply the homogeneous transform to a projective vector into a default vector space if the vector norm is smaller than ${NULL_WEIGHT_TOLERANCE} with a weight management ${WeightManagement.AllPositiveWeights}`, () => {
                const coordinates = [2, 4, 6, 8];
                const vSpace = new ProjectiveRealVectorSpace(dimension, WeightManagement.AllPositiveWeights, true);
                const projRealVector1 = createTestProjectiveRealVector(dimension, vSpace, coordinates, new Weight(NULL_WEIGHT_TOLERANCE / 2, false));
                expect(projRealVector1.dimension).to.eql(dimension);
                expect(projRealVector1.vectorSpace.isDefault).to.eql(true);
                expect(projRealVector1.vectorSpace.weightManagement).to.eql(WeightManagement.AllPositiveWeights);
            ...
            it(`can adjust the tolerance threshold to compute the homogeneous transform of a projective vector into a default vector space with a weight management ${WeightManagement.AllPositiveWeights}`, () => {
                ...
                const projRealVector2 = createTestProjectiveRealVector(dimension, vSpace, coordinates, new Weight(tolerance / 2, false));
...
export function createTestProjectiveRealVector<D extends 3 | 4>(
    dimension: D,
    vectorSpace?: ProjectiveRealVectorSpace<D>,
    coordinates?: number[],
    weight?: Weight
): D extends 3 ? ProjectiveVector2DReal : D extends 4 ? ProjectiveVector3DReal : never;

export function createTestProjectiveRealVector(
    dimension: 3 | 4, vectorSpace?: ProjectiveRealVectorSpace<3 | 4>, coordinates?: number[], weight?: Weight
): ProjectiveVector2DReal | ProjectiveVector3DReal {
    switch(dimension) {
        case 3:
            if(vectorSpace !== undefined && vectorSpace.weightManagement === WeightManagement.AllPositiveWeights) {
            return new ProjectiveVector2DReal(coordinates ? coordinates[0] : defaultCoordinates[0],
                                        coordinates ? coordinates[1] : defaultCoordinates[1],
                                        weight ? weight : defaultPositiveWeight, vectorSpace ? vectorSpace as ProjectiveRealVectorSpace<3>: undefined);
            }
            return new ProjectiveVector2DReal(coordinates ? coordinates[0] : defaultCoordinates[0],
                                        coordinates ? coordinates[1] : defaultCoordinates[1],
                                        weight ? weight : defaultWeight, vectorSpace ? vectorSpace as ProjectiveRealVectorSpace<3>: undefined);
        case 4:
            if(vectorSpace !== undefined && vectorSpace.weightManagement === WeightManagement.AllPositiveWeights) {
            return new ProjectiveVector3DReal(coordinates ? coordinates[0] : defaultCoordinates[0],
                                        coordinates ? coordinates[1] : defaultCoordinates[1],
                                        coordinates ? coordinates[2] : defaultCoordinates[2],
                                        weight ? weight : defaultPositiveWeight, vectorSpace ? vectorSpace as ProjectiveRealVectorSpace<4>: undefined);
            }
            return new ProjectiveVector3DReal(coordinates ? coordinates[0] : defaultCoordinates[0],
                                        coordinates ? coordinates[1] : defaultCoordinates[1],
                                        coordinates ? coordinates[2] : defaultCoordinates[2],
                                        weight ? weight : defaultWeight, vectorSpace ? vectorSpace as ProjectiveRealVectorSpace<4>: undefined);
        default:
