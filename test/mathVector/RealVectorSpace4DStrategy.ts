...
import { REALVECTOR2D, RealVector2D, REALVECTOR3D, RealVector3D, RealVector4D, REALVECTOR4D } from "../../src/mathVector/VectorSpaceConstructorInterface";
...
describe('RealVectorSpac4DStrategy', () => {
    ...
    describe('Methods', () => {
        ...
        it('can get the normalized vector of a RealVector', () => {
            ...
            expect(vec).to.eql(true);
            expect(normalized.coordinates).to.eql([vec1.coordinates[0] / vectorSpace.norm(vec1), vec1.coordinates[1] / vectorSpace.norm(vec1), vec1.coordinates[2] / vectorSpace.norm(vec1), vec1.coordinates[3] / vectorSpace.norm(vec1)]);
        });
        ...
        it('cannot get the cross product of two RealVectors of dimension 4', () => {
            ...
            expect(() => vectorSpace.crossProduct(vec1, vec2)).to.throw(EM_CROSS_PRODUCT_NOT_APPLICABLE_DIM4);
        });
