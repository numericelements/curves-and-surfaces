...
import { BSplineR1toR2DifferentialProperties } from '../../src/bsplines/BSplineR1toR2DifferentialProperties';
...
describe('OptimizationProblemBSplineR1toR2', () => {
    ...
    it('has a number of independent variables equal to twice the number of control points', () => {
        ...
        let o = new OptimizationProblemBSplineR1toR2(splineTarget, splineInitial)
    ...
    it('has a number of constraints + inactiveConstraints equal to (4 * d - 5) * (n - d) for curvature extrema', () => {
        ...
        let o = new OptimizationProblemBSplineR1toR2(splineTarget, splineInitial, ActiveControl.curvatureExtrema)
    ...
    it('returns f of length number of constraints and a gradient_f of shape [number of constraints, number of independent variables]', () => {
        let splineTarget = create_BSplineR1toR2([[-0.5, 0], [-0.25, 0.25], [0.25, 0.25], [0.5, 0]], [0, 0, 0, 0, 1, 1, 1, 1])
        let splineInitial = create_BSplineR1toR2([[-0.5, 0], [-0.25, 0.25], [0.25, 0.25], [0.5, 0]], [0, 0, 0, 0, 1, 1, 1, 1])
        let o = new OptimizationProblemBSplineR1toR2(splineTarget, splineInitial, ActiveControl.both)
    ...
    it('can take a symmetric b-spline curve', () => {
        ...
        splineInitial.insertKnot(0.2)
        splineInitial.insertKnot(0.8)        
        let optimizationProblem = new OptimizationProblemBSplineR1toR2(splineTarget, splineInitial)
