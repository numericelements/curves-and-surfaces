...
describe('Control polygon of vectors of same type and dimension', () => {
    ...
    describe('Methods', () => {
        ...
        it(`can move a control point of a control polygon of real vectors of type ${REALVECTOR1D}`, () => {
            const initCoord = 1;
            const realV1D1 = new Vector1DReal(initCoord);
            const realV1D2 = new Vector1DReal(-1);
            const realV1D3 = new Vector1DReal(0);
            ...
            const dVector = new Vector1DReal(displacement);
            const newCPolygon = cPolygon.withMovedControlPoint(0, dVector);
            expect(newCPolygon.get(0)).to.eql(new Vector1DReal(initCoord + displacement));
        });
    });
});