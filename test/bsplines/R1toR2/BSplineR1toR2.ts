...
describe('BSplineR1toR2', () => {
    ...
    it('can be safely cloned', () => {
        ...
        const cp1 = new Vector2d(0, 8)
        const cp2 = new Vector2d(0.5, 0)
        let s1 = new BSplineR1toR2( [cp0, cp1, cp2], [ 0, 0, 0, 1, 1, 1])
        let s2 = s1.clone()
    ...
    it('can insert a new knot', () => {
        ...
        let s1 = new BSplineR1toR2( [cp0, cp1, cp2], [ 0, 0, 0, 1, 1, 1] )
        let s2 = s1.clone()
        s2 = s2.insertKnot(0.5)
        s2 = s2.insertKnot(0.25)
        s2 = s2.insertKnot(0.75)
        ...
        let spline = new BSplineR1toR2(cp, knots)
    ...
    it('can elevate its degree', () => {
        ...
        //let s2 = s1.generateIntermediateSplinesForDegreeElevation()
        //console.log(s2)
        //console.log(s2.controlPolygons[0])
        //console.log(s2.controlPolygons[1])
        //console.log(s2.controlPolygons[2])
        //let s2 = s1.elevateDegreeJC()
        //s1.elevateDegree()
        //console.log(s2.controlPoints)
        //let s3 = s1.elevateDegreeWithRemoveKnot()
        //expect(s2.degree).equal(3)
        //expect(s3.degree).equal(3)
        //expect(s2).to.deep.equal(s3)
        
        let s2 = s1.elevateDegree()
        //let s3 = s1.elevateDegreeWithRemoveKnot()

        expect(s2.controlPoints).to.deep.equal([
            new Vector2d( -0.5, 0 ),
            new Vector2d( -0.16666666666666666, 5.333333333333333 ),
            new Vector2d( 0.16666666666666666, 5.333333333333333 ),
            new Vector2d(0.5, 0)
          ])
